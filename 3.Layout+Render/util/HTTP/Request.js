...
module.exports = class Request {
     ...
     * 2. 收到数据传给 responseParser
     * 3. 根据 responseParser 的状态 resolve Promise
     */
    send(connection){
        return new Promise((resolve, reject) => {
            ...
            connection.on('data', (data) => {
                ...
                // TODO文章：汉字的.length是1，但是Body里的内容长度汉字长度是3
                // console.log('TCP data LENGTH:', (data.toString()).length);
                // console.log(data.toString());
                responseParser.receive(data.toString());

                console.log('====== ./util/HTTP/Request.js ======\n------ send()');
                if(responseParser.isFinished){
                    console.log('responseParser.isFinished === true');
                    resolve(responseParser.response);
                }else{
                    console.log('responseParser.isFinished === false');
                }
                connection.end();
            });
