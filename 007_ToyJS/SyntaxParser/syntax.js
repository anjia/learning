 ...
 *      与：‘一个’数组
 * 
 * 计算机语言：有了基本的运算能力（表达式）+语句，基本上就图灵完备了（能表达世界上的一切逻辑）
 *      只要有这三种逻辑（顺序执行+分支+循环）存在，语言就能达到图灵完备的状态
 */
export let syntax = {
    Program: [
        ['StatementList', 'EOF']
    ...
    Statement: [  // 语句的返回值~类CompletionRecord
        ['ExpressionStatement'],
        ['VariableDeclaration'],
        ['FunctionDeclaration'],
        ['Block'],
    ...
    BreakStatement: [   // break; continue; 等涉及到“语句返回值”
        ['break', ';']
    ],
    ContinueStatement: [
        ['continue', ';']
